...
from main import ResNetOCTSimCLR, OCTImageDataset, InvalidBackboneError, AdaptiveEqualization, MacularSquareExtractor
...
def get_transform(image_size, macular_size, is_train=True):
    """Get the transformations for evaluation"""
    if is_train:
        return transforms.Compose([
            # First extract the macular square region
            MacularSquareExtractor(square_size=macular_size),
...
def main():
    ...
    parser.add_argument('--macular-size', type=int, default=224,
                        help='Size of the square macular region to extract')
    ...
    # Load the dataset
    train_transform = get_transform(args.image_size, args.macular_size, is_train=True)
    test_transform = get_transform(args.image_size, args.macular_size, is_train=False)
