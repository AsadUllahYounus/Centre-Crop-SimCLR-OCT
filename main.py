...
# MacularSquareExtractor for focusing on central/macular region
class MacularSquareExtractor(object):
    """Extract a square region centered on the macula from OCT images."""
    def __init__(self, square_size=224):
        """
        Args:
            square_size (int): Size of the square region to extract (in pixels)
        """
    ...
    def __call__(self, img):
        ...
        # Calculate the position to crop from - centered in the image
        # For OCT images, the macula is typically in the center
        left = (width - self.square_size) // 2
        top = (height - self.square_size) // 2
        
        # Handle cases where the requested square size is larger than the image
        if self.square_size > min(width, height):
            # Resize the image to fit the square size while maintaining aspect ratio
            ratio = self.square_size / min(width, height)
            new_width = int(width * ratio)
            new_height = int(height * ratio)
            img = img.resize((new_width, new_height), Image.BICUBIC)
            
            # Recalculate crop position
            left = (new_width - self.square_size) // 2
            top = (new_height - self.square_size) // 2
        
        right = left + self.square_size
        bottom = top + self.square_size
        
        # Crop and return the square macular region
        macular_region = img.crop((left, top, right, bottom))
        return macular_region
...
class ContrastiveLearningDataset:
    ...
    @staticmethod
    def get_oct_macular_transform(size):
        """Return a set of data augmentation transformations optimized for macular square regions."""
        # Lighter color jitter for OCT images
        color_jitter = transforms.ColorJitter(brightness=0.4, contrast=0.4)
        
        data_transforms = transforms.Compose([
            # First extract the macular square region
            MacularSquareExtractor(square_size=size),
            # Initial equalization to enhance contrast
            AdaptiveEqualization(),
            # Transformations specifically for the macular region
            transforms.RandomRotation(degrees=5),  # Very small rotations to preserve macular structures
    ...
    def get_dataset(self, name, n_views, image_size=224):
        valid_datasets = {
            ...
            'oct': lambda: OCTImageDataset(
                self.root_folder,
                transform=ContrastiveLearningViewGenerator(
                    self.get_oct_macular_transform(image_size),
                    n_views)
            )
        }
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='PyTorch SimCLR for OCT Macular Regions')
    ...
    parser.add_argument('--image-size', default=224, type=int, help='Size of the macular square to extract from OCT images.')
    parser.add_argument('--macular-size', default=224, type=int, 
                        help='Size of the square macular region to extract in pixels (default: 224)')
...
def main():
    ...
    # Set macular size to image size if not explicitly different
    if args.macular_size != args.image_size:
        args.image_size = args.macular_size
